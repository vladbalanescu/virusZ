...
%migration functions for class zombie
%agt=zombie object
%cn - current agent number

%SUMMARY OF zombie MIGRATE RULE
%If a zombie has not eaten, it will pick a random migration direction
...
mig=0;
cnt=1;
dir=rand*2*pi;              %zombie has been unable to find food, so chooses a random direction to move in
while mig==0&cnt<=8        %zombie has up to 8 attempts to migrate (without leaving the edge of the model)
