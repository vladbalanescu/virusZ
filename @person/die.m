...
%death function for class person
%agt=person object
%cn - current agent number
%klld=1 if agent dies, =0 otherwise

%persons die if their food level reaches zero or they are older than max_age
...
klld=0;
thold=PARAM.P_MINFOOD;      %threshold minimum food value for death to occur
cfood=agt.food;             %get current agent food level
age=agt.age;                %get current agent age

if cfood<=thold|age>PARAM.P_MAXAGE      %if food level < threshold and age > max age then agent dies
